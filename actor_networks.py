...
class ScannedRNN(nn.Module):
  ...
  def __call__(self, carry, x):
    ...
    lstm_state = jax.tree_map(
      lambda x: jnp.where(resets[:, np.newaxis], jnp.zeros_like(x), x), lstm_state
    )
...
class ActorCriticRNN(nn.Module):
  ...
  def __call__(self, hidden, x):
    ...
    if self.config["GRAPH_NET"]:
      ...
      embedding = nn.Conv(
        features=64
        if "9" in self.config["layout_name"]
        else 2 * self.config["FC_DIM_SIZE"],
        kernel_size=(2, 2),
    ...
    #########
    critic = nn.Dense(
      self.config["FC_DIM_SIZE"] * 2, kernel_init=orthogonal(2), bias_init=constant(0.0)
    )(embedding)
