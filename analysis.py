...
async def load_and_analyze_files(extension=""):
  ...
  all_filenames = [
    f for f in all_filenames if (f.endswith(".json") and extension in f)
...
def analyze_single_user(user_data):
  ...
  for datapoint in user_data:
    if "name" not in datapoint:
      continue
    if (
      "ik" in datapoint["name"]
      or "sk" in datapoint["name"]
      or "fcp" in datapoint["name"]
    ):
      if datapoint["metadata"]["type"] != "EnvStage":
        continue
      prev_best_score = success_dict.get(datapoint["name"], 0)
      try:
        success_dict[datapoint["name"]] = max(
          prev_best_score, datapoint["metadata"]["nsuccesses"]
        )
      except:
        pdb.set_trace()
  return success_dict
...
def plot_success_dict(success_dict, extension=""):
  ...
  plt.bar(x_pos, vals, yerr=errs, capsize=5, color="skyblue", alpha=0.8)
